...
const menus = [
    ...
    { href: '/collection', label: '데이터 수집 관리', icon: RefreshCw, roles: ['super_admin', 'manager_full'] },
    { href: '/collection/unmatched', label: '미매칭 데이터', icon: Link2, roles: ['super_admin', 'manager_full'] },
    { href: '/api-keys', label: 'API KEY 관리', icon: KeyRound, roles: ['super_admin', 'manager_full'] },
    { href: '/admins', label: '관리자 관리', icon: Users, roles: ['super_admin'] },
]
...
export default function AdminSidebar() {
    ...
    const visibleMenus = menus.filter((menu) => menu.roles.includes(adminRole))

    const isActive = (href: string) =>
        pathname === href || (href !== '/collection' && pathname.startsWith(`${href}/`))

    return (
        <>
            <div className="lg:hidden border-b border-zinc-200 bg-white px-4 py-3">
